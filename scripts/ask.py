...
def run_analysis(source: str, file, table_name: str, target_col: str):
    ...
    if d.get("columns"):
        lines.append(f"**Numeric columns:** {', '.join(d['columns'])}")
    lines.append("\n### Insights")
    ...
    if target_col:
        ...
        if reg_resp.status_code == 200:
            ...
            try:
                err_detail = reg_resp.json().get("detail", reg_resp.text)
            except Exception:
                err_detail = reg_resp.text
            lines.append(f"\n*Regression failed: {err_detail}*")
            coef_fig  = empty_chart("Regression failed — see message above")
            gauge_fig = empty_chart("Regression failed — see message above")
